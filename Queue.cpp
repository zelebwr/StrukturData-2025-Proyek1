...
int main(){
    ...
    if(!file.is_open()){cerr<<"Error: Failed to open the file.\n";return 1;}
    string line;getline(file,line);
    while(getline(file,line)){
        stringstream ss(line);vector<string> p;string tmp;
    ...
    while(!dataQueue.empty()){
        auto c=dataQueue.front();dataQueue.pop();
        int pC=stoi(c[2]),rC=stoi(c[3]);bool ok=true;
        for(auto&s:skyline){int pS=stoi(s[2]),rS=stoi(s[3]);if(dominates(pS,rS,pC,rC)){ok=false;break;}}
        if(ok){vector<vector<string>> tmp;
            for(auto&s:skyline){int pS=stoi(s[2]),rS=stoi(s[3]);if(!dominates(pC,rC,pS,rS))tmp.push_back(s);}
            tmp.push_back(c);skyline.swap(tmp);
        }
    }
    ...
    int highestRating=INT_MIN;vector<string> best;
    int lowestPrice=INT_MAX;vector<string> cheap;
    for(auto&p:skyline){int price=stoi(p[2]),rating=stoi(p[3]);
        if(rating>highestRating){highestRating=rating;best=p;}
        if(price<lowestPrice){lowestPrice=price;cheap=p;}
    }
    ...
    cout<<"Rating Skyline Time: "<<skylineCalculationDuration.count()<<" seconds\n";
    cout<<"Price Skyline Time: "<<skylineCalculationDuration.count()<<" seconds\n";
